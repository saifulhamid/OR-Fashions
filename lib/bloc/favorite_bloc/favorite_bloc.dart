...
class FavoriteBloc extends Bloc<FavoriteEvent, FavoriteState> {
  ...
  FutureOr<void> toggleFavoriteEvent(
    ...
    try {
      ...
      debugPrint("isFavorite in bloc: ${isFavorite.toString()}");
      if (isFavorite == false) {
      ...
      emit(FavoriteChangedState(isFavorite: !isFavorite, productId: event.productId));
    } catch (e) {
      throw e.toString();
    }
  }

  FutureOr<void> checkIsFavorite(CheckIsFavorite event, Emitter<FavoriteState> emit) async {
    try {
      final isFavorite = await favoriteRepository.isFavorite(productId: event.productId);
      emit(FavoriteChangedState(isFavorite: isFavorite, productId: event.productId));
    } catch (e) {
      throw e.toString();
    }
